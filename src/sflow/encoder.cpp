...
#include "customer_cone/sflow/encoder.hpp"
#include "customer_cone/sflow/sflow.h"      /* sFlow v5 */
#include "customer_cone/sflow/sflow_v2v4.h" /* sFlow v2/4 */

namespace customer_cone {
namespace sflow {
const char* encoder::SFHTTP_method_names[] = {
  "-", "OPTIONS", "GET", "HEAD", "POST", "PUT", "DELETE", "TRACE", "CONNECT"};

const NFField9 encoder::nfField9[]
  = {{ID_SRC_IP, SZ_SRC_IP},
     {ID_DST_IP, SZ_DST_IP},
...
// void (*sendNetFlowDatagram)(SFSample* sample) = sendNetFlowV5Datagram;

int encoder::NFFlowSequenceNo = 0;

u_int8_t encoder::HexLookupL[513] = {
...
uint8_t encoder::HexLookupU[513] = {
  "000102030405060708090A0B0C0D0E0F"
  "101112131415161718191A1B1C1D1E1F"
  "202122232425262728292A2B2C2D2E2F"
  "303132333435363738393A3B3C3D3E3F"
...
uint32_t encoder::MyByteSwap32(uint32_t n) {
  return (((n & 0x000000FF) << 24) + ((n & 0x0000FF00) << 8)
          + ((n & 0x00FF0000) >> 8) + ((n & 0xFF000000) >> 24));
}
uint16_t encoder::MyByteSwap16(uint16_t n) {
  return ((n >> 8) | (n << 8));
}
...
*/
void* encoder::my_calloc(size_t bytes) {
...
void encoder::SFStr_init(SFStr* sb) {
  sb->cap = MAX_STRBUF_LEN;
  sb->len = 0;
  sb->str[0] = '\0';
}

char* encoder::SFStr_str(SFStr* sb) {
  return sb->str;
}

int encoder::SFStr_len(SFStr* sb) {
  return sb->len;
}

int encoder::SFStr_append(SFStr* sb, char* str) {
  if (str == NULL)
    return YES;
  int slen = strlen(str);
...
int encoder::SFStr_append_hex(SFStr* sb, u_char* hex, int nbytes, int prefix,
                              int upper, char sep) {
  if (prefix) {
    if ((sb->cap - sb->len) < 3)
      return NO;
...
int encoder::SFStr_append_array32(SFStr* sb, uint32_t* array32, int n,
...
int encoder::SFStr_append_U32(SFStr* sb, char* fmt, uint32_t val32) {
  char ibuf[200];
  sprintf(ibuf, fmt, val32);
  return SFStr_append(sb, ibuf);
}

int encoder::SFStr_append_U64(SFStr* sb, char* fmt, uint64_t val64) {
  char ibuf[200];
...
int encoder::SFStr_append_double(SFStr* sb, char* fmt, double vald) {
  char ibuf[200];
...
int encoder::SFStr_append_mac(SFStr* sb, uint8_t* mac) {
  return SFStr_append_hex(sb, mac, 6, NO, NO, 0);
}

int encoder::SFStr_append_ip(SFStr* sb, uint8_t* ip) {
  uint32_t array32[4];
  for (int i = 0; i < 4; i++)
    array32[i] = ip[i];
...
int encoder::SFStr_append_ip6(SFStr* sb, uint8_t* ip6) {
  for (int i = 0; i < 16; i += 2) {
    if (i > 0 && SFStr_append(sb, ":") == NO)
      return NO;
...
int encoder::SFStr_append_address(SFStr* sb, SFLAddress* address) {
...
int encoder::SFStr_append_UUID(SFStr* sb, uint8_t* uuid) {
...
int encoder::SFStr_append_tag(SFStr* sb, uint32_t tag) {
  uint32_t parts[2];
  parts[0] = (tag >> 12);
...
int encoder::SFStr_append_timestamp(SFStr* sb, time_t ts) {
  char tstr[200];
...
int encoder::SFStr_append_dataSource(SFStr* sb, uint32_t ds_class,
                                     uint32_t ds_index) {
  char buf[200];
...
int encoder::SFStr_copy(SFStr* sb, char* to, int capacity) {
  int max = capacity - 1;
...
char* encoder::printAddress(SFLAddress* address, SFStr* sb) {
  SFStr_init(sb);
  SFStr_append_address(sb, address);
  return SFStr_str(sb);
}

char* encoder::printMAC(uint8_t* mac, SFStr* sb) {
  SFStr_init(sb);
  SFStr_append_mac(sb, mac);
  return SFStr_str(sb);
}

char* encoder::printTag(uint32_t tag, SFStr* sb) {
  SFStr_init(sb);
  SFStr_append_tag(sb, tag);
  return SFStr_str(sb);
}

char* encoder::printTimestamp(time_t ts, SFStr* sb) {
  SFStr_init(sb);
  SFStr_append_timestamp(sb, ts);
  return SFStr_str(sb);
}

char* encoder::printOUI(uint8_t* oui, SFStr* sb) {
  SFStr_init(sb);
  SFStr_append_hex(sb, oui, 3, NO, YES, '-');
  return SFStr_str(sb);
}

char* encoder::printDataSource(uint32_t ds_class, uint32_t ds_index,
                               SFStr* sb) {
  SFStr_init(sb);
...
char* encoder::printOutputPort_v2v4(uint32_t outputPort, SFStr* sb) {
  SFStr_init(sb);
  if (outputPort & 0x80000000) {
    ...
    SFStr_append_U32(sb, "%u", outputPort);
  return SFStr_str(sb);
}

char* encoder::printInOutPort(uint32_t port, uint32_t format, SFStr* sb) {
  SFStr_init(sb);
  switch (format) {
    case 3:
      SFStr_append_U32(sb, "format==3 %u", port);
      break;
    case 2:
      SFStr_append_U32(sb, "multiple %u", port);
      break;
    case 1:
      SFStr_append_U32(sb, "dropCode %u", port);
      break;
    case 0:
      SFStr_append_U32(sb, "%u", port);
      break;
  }
  return SFStr_str(sb);
}
...
void encoder::json_start(char* fname, char bracket) {
...
void encoder::json_end(char bracket) {
  sfConfig.outputDepth--;
  json_indent();
  printf("%c", bracket);
...
void encoder::json_start_ob(char* fname) {
  json_start(fname, '{');
}
void encoder::json_start_ar(char* fname) {
  json_start(fname, '[');
}
void encoder::json_end_ob() {
  json_end('}');
}
void encoder::json_end_ar() {
  json_end(']');
}
...
void encoder::sf_log_context(SFSample* sample) {
  SFStr agentIP, tag1, tag2, nowstr;
  time_t now = sample->pcapTimestamp ?: sample->readTimestamp;
  printf("%s %s %u %u %u:%u %s %s ", printTimestamp(now, &nowstr),
         printAddress(&sample->agent_addr, &agentIP), sample->agentSubId,
         sample->sequenceNo, sample->s.ds_class, sample->s.ds_index,
         printTag(sample->s.sampleType, &tag1),
         printTag(sample->s.elementType, &tag2));
}

void encoder::sf_log(SFSample* sample, char* fmt, ...) {
...
void encoder::sf_logf(SFSample* sample, char* fieldPrefix, char* fieldName,
                      char* val) {
...
void encoder::sf_logf_U32_formatted(SFSample* sample, char* fieldPrefix,
                                    char* fieldName, char* fmt,
                                    uint32_t val32) {
  SFStr buf;
  SFStr_init(&buf);
...
void encoder::sf_logf_U64_formatted(SFSample* sample, char* fieldPrefix,
                                    char* fieldName, char* fmt,
                                    uint64_t val64) {
  SFStr buf;
  SFStr_init(&buf);
...
void encoder::sf_logf_double_formatted(SFSample* sample, char* fieldPrefix,
                                       char* fieldName, char* fmt,
                                       double vald) {
  SFStr buf;
  SFStr_init(&buf);
...
void encoder::sf_logf_U32(SFSample* sample, char* fieldName, uint32_t val32) {
  sf_logf_U32_formatted(sample, NULL, fieldName, "%u", val32);
}

void encoder::sf_logf_U64(SFSample* sample, char* fieldName, uint64_t val64) {
...
char* encoder::URLEncode(char* in, char* out, int outlen) {
...
int encoder::sampleFilterOK(SFSample* sample) {
  /* the vlan filter will only reject a sample if both in_vlan and out_vlan are
   rejected. If the vlan was not reported in an SFLExtended_Switch struct, but
   was only picked up from the 802.1q header then the out_vlan will be 0,  so to
   be sure you are rejecting vlan 1,  you may need to reject both vlan 0 and
   vlan 1. */
  return (sfConfig.gotVlanFilter == NO || sfConfig.vlanFilter[sample->s.in_vlan]
...
void encoder::writeFlowLine(SFSample* sample) {
  SFStr agentIP, srcMAC, dstMAC, srcIP, dstIP;
  /* source */
  if (printf("FLOW,%s,%d,%d,", printAddress(&sample->agent_addr, &agentIP),
             sample->s.inputPort, sample->s.outputPort)
      < 0) {
    exit(-41);
  }
  /* layer 2 */
  if (printf("%s,%s,0x%04x,%d,%d", printMAC(sample->s.eth_src, &srcMAC),
             printMAC(sample->s.eth_dst, &dstMAC), sample->s.eth_type,
             sample->s.in_vlan, sample->s.out_vlan)
      < 0) {
    exit(-42);
  }
  /* layer 3/4 */
  if (printf(",%s,%s,%d,0x%02x,%d,%d,%d,0x%02x",
             ...
             printAddress(&sample->s.ipdst, &dstIP), sample->s.dcd_ipProtocol,
             sample->s.dcd_ipTos, sample->s.dcd_ipTTL, sample->s.dcd_sport,
             sample->s.dcd_dport, sample->s.dcd_tcpFlags)
      < 0) {
    exit(-43);
  }
  /* bytes */
  if (printf(",%d,%d,%d\n", sample->s.sampledPacketSize,
             sample->s.sampledPacketSize - sample->s.stripped
               - sample->s.offsetToIPV4,
             sample->s.meanSkipCount)
      < 0) {
    exit(-44);
  }
}
...
void encoder::writeLineCustom(SFSample* sample) {
...
void encoder::clearLineCustom(SFSample* sample, EnumSFScope scope) {
  for (int ii = 0; ii < sfConfig.outputFieldList.n; ii++) {
...
void encoder::writeCountersLine(SFSample* sample) {
  /* source */
  SFStr agentIP;
  ...
  if (printf(
        "%u,%u,%" PRIu64 ",%u,%u,%" PRIu64 ",%u,%u,%u,%u,%u,%u,%" PRIu64
        ",%u,%u,%u,%u,%u,%u\n",
        sample->s.ifCounters.ifIndex, sample->s.ifCounters.ifType,
        sample->s.ifCounters.ifSpeed, sample->s.ifCounters.ifDirection,
        sample->s.ifCounters.ifStatus, sample->s.ifCounters.ifInOctets,
        sample->s.ifCounters.ifInUcastPkts,
        sample->s.ifCounters.ifInMulticastPkts,
        sample->s.ifCounters.ifInBroadcastPkts,
        sample->s.ifCounters.ifInDiscards, sample->s.ifCounters.ifInErrors,
        sample->s.ifCounters.ifInUnknownProtos,
        sample->s.ifCounters.ifOutOctets, sample->s.ifCounters.ifOutUcastPkts,
        sample->s.ifCounters.ifOutMulticastPkts,
        sample->s.ifCounters.ifOutBroadcastPkts,
        sample->s.ifCounters.ifOutDiscards, sample->s.ifCounters.ifOutErrors,
        sample->s.ifCounters.ifPromiscuousMode)
      < 0) {
    exit(-46);
  }
}
...
void encoder::receiveError(SFSample* sample, char* errm, int hexdump) {
  SFStr ipbuf;
  SFStr hex;
  char* msg = "";
...
void encoder::lengthCheck(SFSample* sample, char* description, uint8_t* start,
                          int len) {
...
void encoder::decodeLinkLayer(SFSample* sample) {
...
#define IPX_MAX_DATA 546
    ...
    if (ipxChecksum && ipxLen >= IPX_HDR_LEN
  ...
  if (type_len <= NFT_MAX_8023_LEN) {
    ...
    if (ptr[0] == 0xAA && ptr[1] == 0xAA && ptr[2] == 0x03) {
      ...
      ptr += 3;
      /* OUI == 00-00-00 means the next two bytes are the ethernet type (RFC
       * 2895) */
      type_len = (ptr[0] << 8) + ptr[1];
      ptr += 2;
    } else {
...
void encoder::decode80211MAC(SFSample* sample) {
  uint8_t* start = sample->s.header;
  uint8_t* end = start + sample->s.headerLen;
  uint8_t* ptr = start;
  ...
  uint32_t duration_id
    = (ptr[1] << 8) + ptr[0]; /* not in network byte order either? */
  ptr += 2;
  ...
  switch (control) {
   ...
   1      1        RA      TA     DA      SA  (wireless bridge) */
      ...
      if (txMAC)
        sf_logf(sample, NULL, "txMAC", printMAC(txMAC, &buf));
      if (rxMAC)
        sf_logf(sample, NULL, "rxMAC", printMAC(rxMAC, &buf));
    }
  }
}
...
void encoder::decodeIPLayer4(SFSample* sample, uint8_t* ptr) {
...
void encoder::decodeIPV4(SFSample* sample) {
  if (sample->s.gotIPV4) {
    SFStr buf;
    uint8_t* end = sample->s.header + sample->s.headerLen;
    ...
    /* Create a local copy of the IP header (cannot overlay structure in case it
   is not quad-aligned...some
   platforms would core-dump if we tried that).  It's OK coz this probably
   performs just as well anyway. */
    struct myiphdr ip;
...
void encoder::decodeIPV6(SFSample* sample) {
  uint16_t payloadLen;
  uint32_t label;
  uint32_t nextHeader;
  uint32_t tos;
  ...
  if (sample->s.gotIPV6) {
    ...
    if (payloadLen == 0)
      sf_logf(sample, NULL, "IPV6_payloadLen", "<jumbo>");
    else
      sf_logf_U32(sample, "IPV6_payloadLen", payloadLen);
    ...
    { /* src and dst address */
      ...
      ptr += 16;
      sf_logf(sample, NULL, "srcIP6", printAddress(&sample->s.ipsrc, &buf));
      sample->s.ipdst.type = SFLADDRESSTYPE_IP_V6;
      memcpy(&sample->s.ipdst.address, ptr, 16);
      ptr += 16;
      sf_logf(sample, NULL, "dstIP6", printAddress(&sample->s.ipdst, &buf));
    }
 ...
 */
    while (nextHeader == 0 ||  /* hop */
           ...
           /* nextHeader == 50 => encryption - don't bother coz we'll not be
              able to read any further */
           nextHeader == 51 || /* auth */
           nextHeader == 60) { /* destination options */
      uint32_t optionLen, skip;
      sf_logf_U32(sample, "IP6HeaderExtension", nextHeader);
      nextHeader = ptr[0];
      optionLen = 8 * (ptr[1] + 1); /* second byte gives option len in 8-byte
                                       chunks, not counting first 8 */
      skip = optionLen - 2;
      ptr += skip;
      if (ptr > end)
    ...
    /* now that we have eliminated the extension headers, nextHeader should have
   what we want to remember as the ip protocol... */
...
void encoder::readPcapHeader() {
  ...
  if (hdr.magic != TCPDUMP_MAGIC) {
    if (hdr.magic == MyByteSwap32(TCPDUMP_MAGIC)) {
      ...
      fprintf(ERROUT,
              "%s not recognized as a tcpdump file\n(magic number = %08x "
              "instead of %08x)\n",
              sfConfig.readPcapFileName, hdr.magic, TCPDUMP_MAGIC);
      exit(-31);
    }
  }
...
void encoder::writePcapPacket(SFSample* sample) {
  ...
  /* prepare the whole thing in a buffer first, in case we are piping the output
   to another process and the reader_ptr expects it all to appear at once... */
...
static void sendNetFlowDatagram_spoof(SFSample* sample, NFFlowPkt* pkt) {
  ...
  switch (version) {
    case 5: {
      packetLen
        = sizeof(NFFlowPkt5) + sizeof(struct myiphdr) + sizeof(struct myudphdr);
      memcpy(sfConfig.sendPkt.data, (char*)pkt, sizeof(NFFlowPkt5));
    } break;
    case 9: {
      packetLen
        = sizeof(NFFlowPkt9) + sizeof(struct myiphdr) + sizeof(struct myudphdr);
...
void encoder::openNetFlowSocket() {
  int family = (sfConfig.netFlowOutputIP.type == SFLADDRESSTYPE_IP_V6) ?
                 AF_INET6 :
                 AF_INET;
  ...
  /* open the socket */
  if ((sfConfig.netFlowOutputSocket = socket(family, SOCK_DGRAM, IPPROTO_UDP))
      == -1) {
...
void encoder::sendNetFlowV5Datagram(SFSample* sample) {
  NFFlowPkt5 pkt;
  ...
  if (sample->s.udp_pduLen > 0)
    bytes = sample->s.udp_pduLen;
  else
    bytes = sample->s.sampledPacketSize - sample->s.stripped
            - sample->s.offsetToIPV4;
  ...
  pkt.hdr.sysUpTime
    = htonl(now % (3600 * 24))
      * 1000; /* pretend we started at midnight (milliseconds) */
  ...
  if (!sfConfig.disableNetFlowScale) {
    ...
    /* set the sampling_interval header field too (used to be a 16-bit reserved
     * field) */
  ...
  pkt.flow.firstTime
    = pkt.hdr.sysUpTime; /* set the start and end time to be now (in
...
void encoder::sendNetFlowV9Datagram(SFSample* sample) {
  NFFlowPkt9 pkt;
...
uint32_t encoder::getData32_nobswap(SFSample* sample) {
...
uint32_t encoder::getData32(SFSample* sample) {
  return ntohl(getData32_nobswap(sample));
}

float encoder::getFloat(SFSample* sample) {
  float fl;
...
uint64_t encoder::getData64(SFSample* sample) {
...
double encoder::getDouble(SFSample* sample) {
  double dbl;
...
void encoder::skipBytes(SFSample* sample, uint32_t skip) {
  int quads = (skip + 3) / 4;
  sample->datap += quads;
...
uint32_t encoder::sf_log_next32(SFSample* sample, char* fieldName) {
  uint32_t val = getData32(sample);
  sf_logf_U32(sample, fieldName, val);
  return val;
}

uint64_t encoder::sf_log_next64(SFSample* sample, char* fieldName) {
  uint64_t val64 = getData64(sample);
  sf_logf_U64(sample, fieldName, val64);
  return val64;
}

void encoder::sf_log_percentage(SFSample* sample, char* fieldName) {
  char buf[32];
...
float encoder::sf_log_nextFloat(SFSample* sample, char* fieldName) {
  char buf[32];
...
void encoder::sf_log_nextMAC(SFSample* sample, char* fieldName) {
...
uint32_t encoder::getString(SFSample* sample, char* buf, uint32_t bufLen) {
...
uint32_t encoder::getAddress(SFSample* sample, SFLAddress* address) {
...
void encoder::skipTLVRecord(SFSample* sample, uint32_t tag, uint32_t len,
                            char* description) {
  SFStr buf;
  sf_log(sample, "skipping unknown %s: %s len=%d\n", description,
         printTag(tag, &buf), len);
  skipBytes(sample, len);
}
...
void encoder::readExtendedSwitch(SFSample* sample) {
...
void encoder::readExtendedRouter(SFSample* sample) {
  SFStr buf;
  ...
  sf_logf(sample, NULL, "nextHop", printAddress(&sample->s.nextHop, &buf));
...
void encoder::readExtendedGateway_v2(SFSample* sample) {
  ...
  if (sample->s.dst_as_path_len > 0) {
    ...
    sample->s.dst_as
      = ntohl(sample->s.dst_as_path[sample->s.dst_as_path_len - 1]);
  }
  ...
  if (sample->s.dst_as_path_len > 0) {
    ...
    SFStr_append_array32(&dst_as_path, sample->s.dst_as_path,
                         sample->s.dst_as_path_len, YES, '-');
...
void encoder::readExtendedGateway(SFSample* sample) {
  uint32_t segments;
  uint32_t seg;
  SFStr buf;
  ...
  if (sample->datagramVersion >= 5) {
    ...
    sf_logf(sample, NULL, "bgp_nexthop",
            printAddress(&sample->s.bgp_nextHop, &buf));
  }
  ...
  if (sample->s.communities_len > 0)
    sample->s.communities = sample->datap;
  /* and skip over it in the input */
  ...
  if (sample->s.communities_len > 0) {
    ...
    SFStr_append_array32(&communities, sample->s.communities,
                         sample->s.communities_len, YES, '-');
...
void encoder::readExtendedUser(SFSample* sample) {
#define SA_MAX_EXTENDED_USER_LEN 200
  ...
  sf_logf(sample, NULL, "src_user", src_user);
  sf_logf(sample, NULL, "dst_user", dst_user);
}
...
void encoder::readExtendedUrl(SFSample* sample) {
...
#define SA_MAX_EXTENDED_HOST_LEN 200
  ...
  sf_logf(sample, NULL, "url", url);
  if (sample->datagramVersion >= 5) {
    host_len = getString(sample, host, SA_MAX_EXTENDED_HOST_LEN);
    sf_logf(sample, NULL, "host", host);
  }
...
void encoder::mplsLabelStack(SFSample* sample, char* fieldName) {
  SFLLabelStack lstk;
  uint32_t lab;
...
void encoder::readExtendedMpls(SFSample* sample) {
  SFStr buf;
  sf_logf(sample, NULL, "extendedType", "MPLS");
  getAddress(sample, &sample->s.mpls_nextHop);
  sf_logf(sample, NULL, "mpls_nexthop",
          printAddress(&sample->s.mpls_nextHop, &buf));
...
void encoder::readExtendedNat(SFSample* sample) {
  SFStr buf;
  sf_logf(sample, NULL, "extendedType", "NAT");
  getAddress(sample, &sample->s.nat_src);
  sf_logf(sample, NULL, "nat_src", printAddress(&sample->s.nat_src, &buf));
  getAddress(sample, &sample->s.nat_dst);
  sf_logf(sample, NULL, "nat_dst", printAddress(&sample->s.nat_dst, &buf));
...
void encoder::readExtendedNatPort(SFSample* sample) {
...
void encoder::readExtendedMplsTunnel(SFSample* sample) {
...
void encoder::readExtendedMplsVC(SFSample* sample) {
...
void encoder::readExtendedMplsFTN(SFSample* sample) {
...
void encoder::readExtendedMplsLDP_FEC(SFSample* sample) {
...
void encoder::readExtendedVlanTunnel(SFSample* sample) {
  uint32_t lab;
  SFLLabelStack lstk;
  lstk.depth = getData32(sample);
  /* just point at the lablelstack array */
...
void encoder::readExtendedWifiPayload(SFSample* sample) {
...
void encoder::readExtendedWifiRx(SFSample* sample) {
  uint32_t i;
  uint8_t* bssid;
  SFStr buf;
...
void encoder::readExtendedWifiTx(SFSample* sample) {
  uint32_t i;
  uint8_t* bssid;
  SFStr buf;
  char ssid[SFL_MAX_SSID_LEN + 1];
...
void encoder::readFlowSample_header(SFSample* sample) {
  SFStr scratch;
  ...
  if (sample->s.gotIPV4) {
    ...
    sf_logf_U32_formatted(sample, NULL, "IPSize", "%d",
                          sample->s.sampledPacketSize - sample->s.stripped
                            - sample->s.offsetToIPV4);
    decodeIPV4(sample);
...
void encoder::readFlowSample_ethernet(SFSample* sample, char* prefix) {
  SFStr buf;
  SFStr_init(&buf);
  ...
  sf_logf_U32_formatted(sample, prefix, "ethernet_type", "%u",
                        sample->s.eth_type);
  sf_logf_U32_formatted(sample, prefix, "ethernet_len", "%u",
                        sample->s.eth_len);
  sf_logf(sample, prefix, "ethernet_src", printMAC(sample->s.eth_src, &buf));
  sf_logf(sample, prefix, "ethernet_dst", printMAC(sample->s.eth_dst, &buf));
}
...
void encoder::readFlowSample_IPv4(SFSample* sample, char* prefix) {
  SFStr buf;
  SFStr_init(&buf);
  SFStr_append(&buf, prefix);
  ...
  skipBytes(sample, sample->s.headerLen);
  {
    ...
    sf_logf_U32_formatted(sample, prefix, "sampledPacketSize", "%u",
                          sample->s.sampledPacketSize);
    sf_logf_U32_formatted(sample, prefix, "IPSize", "%u",
                          sample->s.sampledPacketSize);
    sample->s.ipsrc.type = SFLADDRESSTYPE_IP_V4;
    ...
    sf_logf(sample, prefix, "srcIP", printAddress(&sample->s.ipsrc, &buf));
    sf_logf(sample, prefix, "dstIP", printAddress(&sample->s.ipdst, &buf));
    sf_logf_U32_formatted(sample, prefix, "IPProtocol", "%u",
                          sample->s.dcd_ipProtocol);
    sf_logf_U32_formatted(sample, prefix, "IPTOS", "%u", sample->s.dcd_ipTos);
    sample->s.dcd_sport = ntohl(nfKey.src_port);
    sample->s.dcd_dport = ntohl(nfKey.dst_port);
    switch (sample->s.dcd_ipProtocol) {
      case 1: /* ICMP */
        sf_logf_U32_formatted(sample, prefix, "ICMPType", "%u",
                              sample->s.dcd_dport);
 ...
 assume that the destination port has the type */
        break;
      case 6: /* TCP */
        sf_logf_U32_formatted(sample, prefix, "TCPSrcPort", "%u",
                              sample->s.dcd_sport);
        sf_logf_U32_formatted(sample, prefix, "TCPDstPort", "%u",
                              sample->s.dcd_dport);
        sample->s.dcd_tcpFlags = ntohl(nfKey.tcp_flags);
        sf_logf_U32_formatted(sample, prefix, "TCPFlags", "%u",
                              sample->s.dcd_tcpFlags);
        break;
      case 17: /* UDP */
        sf_logf_U32_formatted(sample, prefix, "UDPSrcPort", "%u",
                              sample->s.dcd_sport);
        sf_logf_U32_formatted(sample, prefix, "UDPDstPort", "%u",
                              sample->s.dcd_dport);
        break;
      default: /* some other protcol */
        break;
...
void encoder::readFlowSample_IPv6(SFSample* sample, char* prefix) {
  SFStr buf;
  SFStr_init(&buf);
  SFStr_append(&buf, prefix);
  ...
  skipBytes(sample, sample->s.headerLen);
  {
    ...
    sf_logf(sample, prefix, "srcIP6", printAddress(&sample->s.ipsrc, &buf));
    sf_logf(sample, prefix, "dstIP6", printAddress(&sample->s.ipdst, &buf));
    sf_logf_U32_formatted(sample, prefix, "IPProtocol", "%u",
                          sample->s.dcd_ipProtocol);
    sf_logf_U32_formatted(sample, prefix, "priority", "%u",
...
void encoder::readFlowSample_memcache(SFSample* sample) {
...
#define ENC_KEY_BYTES (SFL_MAX_MEMCACHE_KEY * 3) + 1
  ...
  if (getString(sample, key, SFL_MAX_MEMCACHE_KEY) > 0) {
    sf_logf(sample, NULL, "memcache_op_key",
...
void encoder::readFlowSample_http(SFSample* sample, uint32_t tag) {
  ...
  if (sfConfig.outputFormat == SFLFMT_CLF) {
    ...
    char nowstr[200];
    strftime(
      nowstr, 200, "%d/%b/%Y:%H:%M:%S %z",
      localtime(
        &now)); /* there seems to be no simple portable equivalent to %z */
    /* should really be: snprintf(sfCLF.http_log, SFLFMT_CLF_MAX_LINE,...) but
     * snprintf() is not always available */
    sprintf(sfCLF.http_log,
            "- %s [%s] \"%s %s HTTP/%u.%u\" %u %" PRIu64 " \"%s\" \"%s\"",
            authuser[0] ? authuser : "-", nowstr, SFHTTP_method_names[method],
            uri[0] ? uri : "-", protocol / 1000, protocol % 1000, status,
            resp_bytes, referrer[0] ? referrer : "-",
...
void encoder::readFlowSample_APP(SFSample* sample) {
...
void encoder::readFlowSample_APP_CTXT(SFSample* sample) {
  char application[SFLAPP_MAX_APPLICATION_LEN];
  char operation[SFLAPP_MAX_OPERATION_LEN];
  char attributes[SFLAPP_MAX_ATTRIBUTES_LEN];
  if (getString(sample, application, SFLAPP_MAX_APPLICATION_LEN) > 0) {
...
void encoder::readFlowSample_APP_ACTOR_INIT(SFSample* sample) {
...
void encoder::readFlowSample_APP_ACTOR_TGT(SFSample* sample) {
  char actor[SFLAPP_MAX_ACTOR_LEN];
  if (getString(sample, actor, SFLAPP_MAX_ACTOR_LEN) > 0) {
...
void encoder::readExtendedSocket4(SFSample* sample) {
  SFStr buf;
  ...
  sf_logf(sample, NULL, "socket4_local_ip",
          printAddress(&sample->s.ipsrc, &buf));
  sf_logf(sample, NULL, "socket4_remote_ip",
          printAddress(&sample->s.ipdst, &buf));
...
void encoder::readExtendedProxySocket4(SFSample* sample) {
  SFStr buf;
...
void encoder::readExtendedSocket6(SFSample* sample) {
  SFStr buf;
  ...
  skipBytes(sample, 16);
  sf_logf(sample, NULL, "socket6_local_ip",
          printAddress(&sample->s.ipsrc, &buf));
  sf_logf(sample, NULL, "socket6_remote_ip",
          printAddress(&sample->s.ipdst, &buf));
...
void encoder::readExtendedProxySocket6(SFSample* sample) {
  SFStr buf;
  SFLAddress ipsrc, ipdst;
...
void encoder::readExtendedDecap(SFSample* sample, char* prefix) {
  SFStr buf;
  SFStr_init(&buf);
  SFStr_append(&buf, prefix);
  ...
  sf_logf_U32_formatted(sample, prefix, "decap_inner_header_offset", "%u",
                        offset);
}
...
void encoder::readExtendedVNI(SFSample* sample, char* prefix) {
  SFStr buf;
  SFStr_init(&buf);
  SFStr_append(&buf, prefix);
...
void encoder::readExtendedTCPInfo(SFSample* sample) {
  char* direction;
  EnumPktDirection dirn = (EnumPktDirection)getData32(sample);
  switch (dirn) {
    case PKTDIR_unknown:
      direction = "unknown";
      break;
    case PKTDIR_received:
      direction = "received";
      break;
    case PKTDIR_sent:
      direction = "sent";
...
void encoder::readExtendedEntities(SFSample* sample) {
...
void encoder::readFlowSample_v2v4(
  SFSample* sample,
  ...
  sf_logf_U32(sample, "sampleSequenceNo", sample->s.samplesGenerated);
  {
    ...
    sf_logf(sample, NULL, "sourceId",
            printDataSource(sample->s.ds_class, sample->s.ds_index, &buf));
  }
  ...
  sf_logf(sample, NULL, "outputPort",
          printOutputPort_v2v4(sample->s.outputPort, &buf));
  ...
  if (sampleFilterOK(sample)) {
    switch (sfConfig.outputFormat) {
      case SFLFMT_NETFLOW:
        ...
        if (sfConfig.netFlowOutputSocket
            && (sample->s.gotIPV4 || sample->s.gotIPV4Struct))
    ...
    if (sample->s.ifCounters.ifIndex == 0) {
      auto s = make_sample(sample);
      if (s)
        samples.push_back(s);
    }
  }
...
void encoder::readFlowSample(
  SFSample* sample, int expanded,
  std::vector<std::shared_ptr<customer_cone::sample>>& samples) {
  SFStr buf;
  ...
  sf_logf(sample, NULL, "inputPort",
          printInOutPort(sample->s.inputPort, sample->s.inputPortFormat, &buf));
  sf_logf(
    sample, NULL, "outputPort",
    printInOutPort(sample->s.outputPort, sample->s.outputPortFormat, &buf));
  ...
  num_elements = getData32(sample);
  {
    ...
    for (el = 0; el < num_elements; el++) {
      ...
      switch (tag) {
        case SFLFLOW_HEADER:
          // sfsamples.push_back(std::make_unique<customer_cone::sample>(readFlowSample_header(sample)));
          readFlowSample_header(sample);
          break;
        case SFLFLOW_ETHERNET:
          ...
          break;
        case SFLFLOW_IPV4:
          // sfsamples.push_back(std::make_unique<customer_cone::sample>(readFlowSample_IPv4(sample,
          // "")));
          readFlowSample_IPv4(sample, "");
          break;
        case SFLFLOW_IPV6:
        ...
        case SFLFLOW_EX_80211_TX:
          ...
          break;
          /* case SFLFLOW_EX_AGGREGATION: readExtendedAggregation(sample);
           * break; */
        ...
        case SFLFLOW_EX_TCP_INFO:
          readExtendedTCPInfo(sample);
          break;
        case SFLFLOW_EX_ENTITIES:
          readExtendedEntities(sample);
          break;
        default:
  ...
  if (sampleFilterOK(sample)) {
    switch (sfConfig.outputFormat) {
      case SFLFMT_NETFLOW:
        /* if we are exporting netflow and we have an IPv4 layer, compose the
         * datagram now */
        if (sfConfig.netFlowOutputSocket && sample->s.gotIPV4)
          sendNetFlowDatagram(sample);
        break;
      case SFLFMT_PCAP:
        /* if we are writing tcpdump format, write the next packet record now */
...
encoder::make_sample(const SFSample* sf_sample) const {
  timeval t{};
  t.tv_sec = sf_sample->pcapTimestamp ? sf_sample->pcapTimestamp :
                                        sf_sample->readTimestamp;
  t.tv_usec = 0;

  auto buf = new uint8_t[sf_sample->s.sampledPacketSize];
  memset(buf, 0, sf_sample->s.sampledPacketSize);
  memcpy(buf, sf_sample->s.header, sf_sample->s.headerLen);

  pcpp::RawPacket p{buf, static_cast<int>(sf_sample->s.sampledPacketSize), t,
                    true};

  pcpp::Packet parsedPacket(&p);
  auto tmp = std::make_unique<customer_cone::sample>(
    parsedPacket, sf_sample->s.meanSkipCount);
  tmp->vlan_in = sf_sample->s.in_vlan;
  tmp->vlan_out = sf_sample->s.out_vlan;
  tmp->sampled_packet_size = sf_sample->s.sampledPacketSize;
  return tmp;
}
...
void encoder::readCounters_generic(SFSample* sample) {
  /* the first part of the generic counters block is really just more info about
   * the interface. */
  ...
  sample->s.ifCounters.ifInMulticastPkts
    = sf_log_next32(sample, "ifInMulticastPkts");
  sample->s.ifCounters.ifInBroadcastPkts
    = sf_log_next32(sample, "ifInBroadcastPkts");
  ...
  sample->s.ifCounters.ifInUnknownProtos
    = sf_log_next32(sample, "ifInUnknownProtos");
  ...
  sample->s.ifCounters.ifOutMulticastPkts
    = sf_log_next32(sample, "ifOutMulticastPkts");
  sample->s.ifCounters.ifOutBroadcastPkts
    = sf_log_next32(sample, "ifOutBroadcastPkts");
  ...
  sample->s.ifCounters.ifPromiscuousMode
    = sf_log_next32(sample, "ifPromiscuousMode");
}
...
void encoder::readCounters_ethernet(SFSample* sample) {
...
void encoder::readCounters_tokenring(SFSample* sample) {
...
void encoder::readCounters_vg(SFSample* sample) {
...
void encoder::readCounters_vlan(SFSample* sample) {
  sample->s.in_vlan = getData32(sample);
  sf_logf_U32(sample, "in_vlan", sample->s.in_vlan);
...
void encoder::readCounters_80211(SFSample* sample) {
...
void encoder::readCounters_processor(SFSample* sample) {
...
void encoder::readCounters_radio(SFSample* sample) {
...
void encoder::readCounters_OFPort(SFSample* sample) {
...
void encoder::readCounters_portName(SFSample* sample) {
...
void encoder::readCounters_OVSDP(SFSample* sample) {
...
void encoder::readCounters_host_hid(SFSample* sample) {
  uint32_t i;
  uint8_t* uuid;
...
void encoder::readCounters_adaptors(SFSample* sample) {
  uint8_t* mac;
...
void encoder::readCounters_host_parent(SFSample* sample) {
...
void encoder::readCounters_host_cpu(SFSample* sample, uint32_t length) {
...
void encoder::readCounters_host_mem(SFSample* sample) {
...
void encoder::readCounters_host_dsk(SFSample* sample) {
...
void encoder::readCounters_host_nio(SFSample* sample) {
...
void encoder::readCounters_host_ip(SFSample* sample) {
...
void encoder::readCounters_host_icmp(SFSample* sample) {
...
void encoder::readCounters_host_tcp(SFSample* sample) {
...
void encoder::readCounters_host_udp(SFSample* sample) {
...
void encoder::readCounters_host_vnode(SFSample* sample) {
...
void encoder::readCounters_host_vcpu(SFSample* sample) {
...
void encoder::readCounters_host_vmem(SFSample* sample) {
...
void encoder::readCounters_host_vdsk(SFSample* sample) {
...
void encoder::readCounters_host_vnio(SFSample* sample) {
...
void encoder::readCounters_host_gpu_nvml(SFSample* sample) {
...
void encoder::readCounters_bcm_tables(SFSample* sample) {
...
void encoder::readCounters_memcache(SFSample* sample) {
...
void encoder::readCounters_memcache2(SFSample* sample) {
  sf_log_next32(sample, "memcache_cmd_set");
...
void encoder::readCounters_http(SFSample* sample) {
...
void encoder::readCounters_JVM(SFSample* sample) {
...
void encoder::readCounters_JMX(SFSample* sample, uint32_t length) {
...
void encoder::readCounters_APP(SFSample* sample) {
  char application[SFLAPP_MAX_APPLICATION_LEN];
  if (getString(sample, application, SFLAPP_MAX_APPLICATION_LEN) > 0) {
    sf_logf(sample, NULL, "application", application);
  }
...
void encoder::readCounters_APP_RESOURCE(SFSample* sample) {
...
void encoder::readCounters_APP_WORKERS(SFSample* sample) {
...
void encoder::readCounters_VDI(SFSample* sample) {
...
void encoder::readCounters_LACP(SFSample* sample) {
...
void encoder::sf_logf_SFP(SFSample* sample, char* field, uint32_t lane,
                          uint32_t val32) {
  if (sfConfig.outputFormat == SFLFMT_JSON) {
...
void encoder::readCounters_SFP(SFSample* sample) {
...
void encoder::readCountersSample_v2v4(SFSample* sample) {
  SFStr buf;
...
void encoder::readCountersSample(SFSample* sample, int expanded) {
  SFStr buf;
  ...
  for (uint32_t el = 0; el < num_elements; el++) {
    ...
    switch (tag) {
      ...
      case SFLCOUNTERS_OFPORT:
        readCounters_OFPort(sample);
        break;
      case SFLCOUNTERS_PORTNAME:
        readCounters_portName(sample);
        break;
      ...
      case SFLCOUNTERS_HOST_CPU:
        readCounters_host_cpu(sample, length);
        break;
      ...
      case SFLCOUNTERS_HOST_IP:
        readCounters_host_ip(sample);
        break;
      case SFLCOUNTERS_HOST_ICMP:
        readCounters_host_icmp(sample);
        break;
      case SFLCOUNTERS_HOST_TCP:
        readCounters_host_tcp(sample);
        break;
      case SFLCOUNTERS_HOST_UDP:
        readCounters_host_udp(sample);
        break;
      ...
      case SFLCOUNTERS_BCM_TABLES:
        readCounters_bcm_tables(sample);
        break;
      ...
      case SFLCOUNTERS_OVSDP:
        readCounters_OVSDP(sample);
        break;
      default:
        skipTLVRecord(sample, tag, length, "counters_sample_element");
        break;
    }
...
void encoder::readRTMetric(SFSample* sample) {
    ...
    for (el = 0; el < num_elements; el++) {
      ...
      switch (mtype) {
        ...
        case 1:
          ...
          sf_logf_U32_formatted(sample, "rtmetric_counter32_", mname, "%u",
                                mvali32);
          break;
        case 2:
          mvali64 = getData64(sample);
          sf_logf_U64_formatted(sample, "rtmetric_counter64_", mname,
                                "%" PRIu64, mvali64);
          break;
        case 3:
          mvali32 = getData32(sample);
          sf_logf_U32_formatted(sample, "rtmetric_gauge32_", mname, "%u",
                                mvali32);
          break;
        case 4:
          mvali64 = getData64(sample);
          sf_logf_U64_formatted(sample, "rtmetric_gauge64_", mname, "%" PRIu64,
                                mvali64);
          break;
        case 5:
          mvalfloat = getFloat(sample);
          sf_logf_double_formatted(sample, "rtmetric_gaugeFloat_", mname,
                                   "%.3f", (double)mvalfloat);
          break;
        case 6:
          mvaldouble = getDouble(sample);
          sf_logf_double_formatted(sample, "rtmetric_gaugeDouble_", mname,
                                   "%.3f", mvaldouble);
          break;
        default:
...
void encoder::readRTFlow(SFSample* sample) {
  char dsName[SFL_MAX_RTMETRIC_KEY_LEN];
  uint32_t sampleLength;
  uint32_t num_elements;
  uint8_t* sampleStart;
    ...
    for (el = 0; el < num_elements; el++) {
      ...
      switch (ftype) {
        ...
        case 2:
          ...
          sf_logf(sample, "rtflow_ip_", fname,
                  printAddress(&fvaladdr, &addrstr));
          break;
        case 3:
          fvaladdr.type = SFLADDRESSTYPE_IP_V6;
          memcpy(fvaladdr.address.ip_v6.addr, sample->datap, 16);
          skipBytes(sample, 16);
          sf_logf(sample, "rtflow_ip6_", fname,
                  printAddress(&fvaladdr, &addrstr));
          break;
        case 4:
        ...
        case 5:
          ...
          sf_logf_U64_formatted(sample, "rtflow_int64_", fname, "%" PRIu64,
                                fvali64);
          break;
        case 6:
          fvalfloat = getFloat(sample);
          sf_logf_double_formatted(sample, "rtflow_float_", fname, "%.3f",
                                   (double)fvalfloat);
          break;
        case 7:
          fvaldouble = getDouble(sample);
          sf_logf_double_formatted(sample, "rtflow_double_", fname, "%.3f",
                                   fvaldouble);
          break;
        default:
...
encoder::readSFlowDatagram(SFSample* sample) {
  ...
  sf_logf(sample, NULL, "datagramSourceIP",
  ...
  sf_logf(sample, NULL, "localtime",
          printTimestamp(sample->readTimestamp, &buf));
  if (sample->pcapTimestamp) {
    /* thanks to Richard Clayton for this bugfix */
    sf_logf(sample, NULL, "pcapTimestamp",
            printTimestamp(sample->pcapTimestamp, &buf));
  }

  /* check the version */
  sample->datagramVersion = getData32(sample);
  sf_logf_U32(sample, "datagramVersion", sample->datagramVersion);
  if (sample->datagramVersion != 2 && sample->datagramVersion != 4
  ...
  sample->sequenceNo
    = getData32(sample); /* this is the packet sequence number */
  ...
  sfConfig.currentFieldScope = SFSCOPE_SAMPLE;
  {
    ...
    for (; samp < samplesInPacket; samp++) {
      ...
      sf_logf(sample, NULL, "sampleType_tag",
              printTag(sample->s.sampleType, &buf));
      if (sample->datagramVersion >= 5) {
        switch (sample->s.sampleType) {
          case SFLFLOW_SAMPLE:
            readFlowSample(sample, NO, samples);
            break;
          case SFLCOUNTERS_SAMPLE:
            readCountersSample(sample, NO);
            break;
          case SFLFLOW_SAMPLE_EXPANDED:
            readFlowSample(sample, YES, samples);
            break;
          ...
          case SFLRTFLOW:
            ...
            break;
          default:
            skipTLVRecord(sample, sample->s.sampleType, getData32(sample),
                          "sample");
            break;
        }
      } else {
        switch (sample->s.sampleType) {
          case FLOWSAMPLE:
            readFlowSample_v2v4(sample, samples);
            break;
          case COUNTERSSAMPLE:
...
std::vector<std::shared_ptr<sample>>
encoder::receiveSFlowDatagram(SFSample* sample) {
  if (sfConfig.forwardingTargets || sfConfig.forwardingTargets6) {
   ...
   be important from a performance point of view). */
    ...
    for (; tgt != nullptr; tgt = tgt->nxt) {
      int bytesSent;
      if ((bytesSent = sendto(
             tgt->sock, (const char*)sample->rawSample, sample->rawSampleLen, 0,
             (struct sockaddr*)(&tgt->addr), sizeof(tgt->addr)))
          != sample->rawSampleLen) {
        fprintf(ERROUT, "sendto returned %d (expected %d): %s\n", bytesSent,
                sample->rawSampleLen, strerror(errno));
      }
    }
    ...
    for (; tgt6 != NULL; tgt6 = tgt6->nxt) {
      int bytesSent;
      if ((bytesSent = sendto(
             tgt6->sock, (const char*)sample->rawSample, sample->rawSampleLen,
             0, (struct sockaddr*)(&tgt6->addr), sizeof(tgt6->addr)))
          != sample->rawSampleLen) {
        fprintf(ERROUT, "sendto returned %d (expected %d): %s\n", bytesSent,
                sample->rawSampleLen, strerror(errno));
      }
    ...
    if ((exceptionVal = setjmp(sample->env)) == 0) {
      ...
      return readSFlowDatagram(sample);
      // return sample;
    } else {
      /* CATCH */
  ...
  return std::vector<std::shared_ptr<customer_cone::sample>>();
}
...
int encoder::openInputUDPSocket(uint16_t port) {
  int soc;
  ...
  /* Bind the socket */
  if (bind(soc, (struct sockaddr*)&myaddr_in, sizeof(struct sockaddr_in))
      == -1) {
...
int encoder::openInputUDP6Socket(uint16_t port) {
  int soc;
  ...
  /* Bind the socket */
  if (bind(soc, (struct sockaddr*)&myaddr_in6, sizeof(struct sockaddr_in6))
      == -1) {
...
int encoder::ipv4MappedAddress(SFLIPv6* ipv6addr, SFLIPv4* ip4addr) {
...
void encoder::readPacket(int soc) {
  ...
  cc = recvfrom(soc, buf, SA_MAX_SFLOW_PKT_SIZ, 0, (struct sockaddr*)&peer,
                &alen);
  if (cc <= 0) {
...
int encoder::pcapOffsetToSFlow(uint8_t* start, int len) {
  ...
  while (type_len == 0x8100 || type_len == 0x9100) {
    /* VLAN  - next two bytes */
    /*  _____________________________________ */
    /* |   pri  | c |         vlan-id        | */
    /*  ------------------------------------- */
...
encoder::readPcapPacket(FILE* file) {
  std::vector<SFSample*> sfsamples;
  ...
  if (fread(&hdr, sizeof(hdr), 1, file) != 1) {
    if (feof(file))
      //      fprintf(ERROUT, "unable to read pcap packet header from %s :
      //      %s\n",
      //              sfConfig.readPcapFileName, strerror(errno)
      // std::free(sample);
      return caf::none;
  ...
  if (hdr.caplen > SA_MAX_PCAP_PKT) {
    //    fprintf(ERROUT, "pcap %s : capture-len (%u) too long for read
    //    buffer\n",
  ...
  if (hdr.caplen < hdr.len) {
   ...
   for now we just assume ethernet + IP + UDP */
    ...
    return receiveSFlowDatagram(&sample);
  }
}
...
void encoder::peekForNumber(char* p) {
  if (*p < '0' || *p > '9') {
...
void encoder::testVlan(uint32_t num) {
...
void encoder::parseVlanFilter(uint8_t* array, uint8_t flag, char* start) {
  char* p = start;
  char* sep = " ,";
  do {
    uint32_t first, last;
...
int encoder::parseFields(char* start, char** array) {
  char* p = start;
  char* sep = " ,";
  int tokens = 0;
  do {
...
void encoder::parseFieldList(SFFieldList* fieldList, char* start) {
...
int encoder::parseOrResolveAddress(char* name, struct sockaddr* sa,
                                   SFLAddress* addr, int family, int numeric) {
  struct addrinfo* info = NULL;
  struct addrinfo hints = {0};
  hints.ai_socktype
    = SOCK_DGRAM;           /* constrain this so we don't get lots of answers */
...
/*_________________---------------------------__________________
 ...
 void encoder::process_command_line(int argc, char* argv[]) {
  int arg = 1, in = 0;
  int i;
  int plus, minus;
  size_t len_str;
  ...
  while (arg < argc) {
    ...
    switch (in) {
      ...
      case 'x':
        ...
        break;
      case 'c':
        if (setNetFlowCollector(argv[arg++]) == NO)
          exit(-8);
      ...
      case 'v':
        if (plus) {
          ...
          if (!sfConfig.gotVlanFilter) {
            /* when we start with an exclude list, that means the default should
be YES for (i = 0; i < FILTER_MAX_VLAN; i++) sfConfig.vlanFilter[i] = YES;
            sfConfig.gotVlanFilter = YES;
          }
...
open the input socket -- for now it's either a v4 or v6 socket, but in
   future we may allow both to be opened so that platforms that refuse to allow
   v4 packets to be received on a v6 socket can still get both. I think for that
   to really work, however,  we will probably need to allow the bind() to be on
   a particular v4 or v6
...
set the select mask
  loop reading packets
    for (;;) {
      ...
      nfds = select((soc4 > soc6 ? soc4 : soc6) + 1, &readfds, (fd_set*)NULL,
                    (fd_set*)NULL, &timeout);
